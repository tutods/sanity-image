import { SANITY_IMAGE_ID_PATTERN } from "@/constants";
 ...
 */
const parseImageId = (id: string): ImageIdParts => {
 ...
 */
const imageIdToUrlPath = (id: string): string => {
  ...
  return `${id.slice(6, formatSeparatorIndex)}.${id.slice(formatSeparatorIndex + 1)}`;
};

export { imageIdToUrlPath, parseImageId };
