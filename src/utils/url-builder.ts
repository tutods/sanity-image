...
import { imageIdToUrlPath, parseImageId } from '~/utils/parse-image-id';

/**
 * Convert ImageSrcInputs into a full image URL and computed output dimensions.
 */
export const buildSrc = ({ baseUrl, ...inputParams }: ImageSrcInputs): ComputedImageData => {
...
export const buildSrcSet = ({
  ...
  const { h, w } = buildQueryParams({ crop, height, hotspot, id, mode, width });
...
export const buildSvgAttributes = ({ baseUrl, id }: ImageSrcInputs) => {
...
export const croppedImageSize = (
  ...
  if (crop.left + crop.right >= 1 || crop.top + crop.bottom >= 1) {
    throw new Error(`Invalid crop: ${JSON.stringify(crop)}; crop values must be less than 1`);
  }
...
export const buildRect = (
  ...
  const { height, width } = croppedImageSize(dimensions, crop);
