...
describe('without preview', () => {
  it('renders an image tag', () => {
    const { baseElement } = render(<SanityImage id={id} width={500} baseUrl={baseUrl} />);

    const { alt, height, loading, src, srcset, width } = getAttributes(baseElement);

    expect(src).toEqual('/images/abc123-1000x1000.jpg?auto=format&fit=max&q=75&w=500');
  ...
  it('passes through extra attributes', () => {
    const { baseElement } = render(
      <SanityImage id={id} width={500} baseUrl={baseUrl} aria-label="Woo" className="big-ol-img" />,
    );
  ...
  it('overrides default attributes', () => {
    ...
    const { alt, height, id: idAttr, loading, width } = getAttributes(baseElement);
  ...
  it('works with projectId and dataset', () => {
    const { baseElement } = render(
      <SanityImage id={id} width={100} projectId="abc123" dataset="production" />,
    );
  ...
  it('respects queryParams', () => {
    const { baseElement } = render(
      <SanityImage id={id} width={500} baseUrl={baseUrl} queryParams={{ blur: 220, q: 30 }} />,
    );

    const { src, srcset } = getAttributes(baseElement);

    expect(src).toEqual('/images/abc123-1000x1000.jpg?auto=format&blur=220&fit=max&q=30&w=500');
    expect(srcset).toEqual(
...
describe('cursed situations', () => {
  ...
  it('throws if no baseUrl/projectId/dataset are provided', () => {
    ...
    }).toThrow('Missing required `baseUrl` or `projectId` and `dataset` props for <SanityImage>.');
  });
});
...
describe('svg source image', () => {
  it('skips transformations', () => {
    ...
    const { alt, height, src, srcset, width } = getAttributes(baseElement);
